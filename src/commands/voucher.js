...
function voucher (message, deeponionNode, voucherCode) {
  ...
  if (message.channel.type === 'dm') {
    ...
    if (validVoucher) {
      deeponionNode.getBalance('', function (err, balance) {
        if (err) {
          console.log(err)
        ...
        deeponionNode.move('', account, 30, function (err, result) {
          if (err) {
            console.log(err)
            message.channel.send(OOPS_TEXT)
            return
