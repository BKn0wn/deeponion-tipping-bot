...
const NO_FUNDS = 'You dont have onion to transfer.'
const NOT_ENOUGH_FUNDS = 'Not enough funds for this transfer. Please add some onions.'

function withdraw (message, deeponionNode, amount, toAddress) {
  ...
  deeponionNode.getBalance(fromAccount, function (err, balance) {
    if (err) {
      console.log(err)
    ...
    deeponionNode.sendfrom(fromAccount, toAddress, amountInt, function (err, result) {
      if (err) {
        console.log(err)
        message.channel.send(OOPS_TEXT)
        return
